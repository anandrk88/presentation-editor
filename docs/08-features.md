...
- **Legend** (no clipping when there's room): a **left/right** column auto-sizes to the longest entry (capped at 45% of the chart width so a long name can't starve the plot); a **top/bottom** legend measures the entries and shows them in full when they fit the chart width, truncating to equal slots only when they don't. All derived from the labels, so it re-fits on reopen — no round-trip needed.
